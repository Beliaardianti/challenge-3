...
const InputContactForm = (props) => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      handleGetContacts(); // Memperbarui daftar kontak setelah menambahkan kontak baru
      resetInputValue(); // Mengosongkan field input
    } catch (error) {
      console.error("Error submitting contact:", error); // Menangani kesalahan jika terjadi
    }
  };
  

  const allowSubmit = fullName && phoneNumber && email; // Memastikan semua field diisi
  ...
  return (
    <div className="input-contact__form-container">
      ...
      <div className="input-contact__form">
        ...
        <button
  onClick={handleSubmit} // Menambahkan ini untuk memanggil handleSubmit saat tombol diklik
  disabled={!allowSubmit} // Mengatur tombol agar hanya bisa diklik jika semua input diisi
  data-cy="btn-submit"
>
  Simpan
</button>
